...
func _physics_process(delta: float) -> void:
	...
	if dist_to_player <= chase_player_radius:
		change_state(initial_pattern)
	else:
		change_state(states.IDLE)
	...
	match current_state:
		states.IDLE:
			nav_Agent.target_position = global_position  # Idle bleibt stehen
		states.FOLLOWING:
	...
	if player and chase_player_radius <= dist_to_player:
		time_since_last_change +=delta
		if time_since_last_change >= idle_rotation_time:
			time_since_last_change = 0.0
			var current       = $"../tower".rotation
			target_angle = current + randf_range(PI, -PI)
		var target: float = lerp_angle($"../tower".rotation, target_angle, delta * 5/turret_rotation_speed)
		input_rotation = Vector2.UP.rotated(target)
		input_rotation_changed.emit(input_rotation)
		
	elif player and chase_player_radius > dist_to_player:
		var current       = $"../tower".rotation
		target_angle = (player.global_position - $"..".global_position).angle() + PI / 2
		var target: float = lerp_angle(current, target_angle, delta * 5/turret_rotation_speed)
		input_rotation = Vector2.UP.rotated(target)
		input_rotation_changed.emit(input_rotation)
		shoot.emit()
